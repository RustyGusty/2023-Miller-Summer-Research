...
set(handles.filename_posd, 'String', get(handles.positiond, 'String'));
...
if isequal(get(handles.autosave,'Value'),0) % If autoset is not tick
    ...
    if isempty(handles.filenumber) % If filename was not set
        ...
        if(handles.imageisbg) % Show image name
            ...
            image = [handles.root '_' stage '_' handles.filenumber '.tif']; % If not BG, include DS
        end
        notes = ' '; % Show notes to empty String
        ...
        if ( get(handles.goscan, 'Value') ) 
            if( not(exist([handles.path handles.run handles.scannumber '\' stage '\'], 'dir')) )
                system(['mkdir ' handles.path handles.run '1\' stage]);
                system(['mkdir ' handles.path handles.run '2\' stage]);
            end
            imgfilename = [handles.path handles.run handles.scannumber '\' stage '\' ...
                handles.root '_' stage '_' handles.filenumber '.tif'];
        elseif ( handles.imageisbg ) % If this is a background image
            ...
            imgfilename = [handles.path handles.root '_' stage '_' handles.filenumber '.tif'];
        end
...
%     end
    ...
    for i = DS_init:step_size:DS_final
        ...
        compute_avg(handles, [mainpath '1\' get(handles.positiond, 'String') '\'], [handles.root '_' stage '_'], ...
            last_img - 1:-2:(last_img - nscans), [mainpath '1\' handles.root '_' stage '_avg.tif'] );
        compute_avg(handles, [mainpath '2\' get(handles.positiond, 'String') '\'], [handles.root '_' stage '_'], ...
            last_img:-2:(last_img - nscans + 1), [mainpath '2\' handles.root '_' stage '_avg.tif'] );
